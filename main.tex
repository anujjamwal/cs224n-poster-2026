...
\usepackage{booktabs}
\usepackage{tikz}
\usetikzlibrary{arrows, positioning, calc} % Changed to 'arrows'
\usepackage{pgfplots}
...
\begin{column}{\colwidth}
  ...
  \begin{block}{Introduction}
    ...
    Another problem arises in the domain of tasks where the context, if unchecked, can grow infinitely with time like Coding tasks, Control tasks like controlling GUI or Robot motion.

  \end{block}

  \begin{block}{Background: Context Window}
    There are categories of problems where context window growth poses a problem
    \begin{itemize}
        \item \textbf{Context Augmentative Task:} These are tasks that operate on context and as such need to import large chunks of information into the context. These include RAG, Code understanding and Data Analysis tasks.
        \item \textbf{Long Horizon Tasks:} These tasks operate for a long period of time and generally involve tool invocation at different points during the inference. These tasks include Robotic Control, GUI Manipulation, Coding and "world" exploration to name a few.
    \end{itemize}
    
    In both category of tasks, the context length grows until it eventually runs out of memory. All these tasks require a long context window, however this need is transitory and once the subproblem is resolved, the context becomes redundant.
    \end{block}
  
    \begin{block}{Background: Redundancies in CoT}
        Modern LLMs prefer lengthy CoT's. These CoT's are full of redundancies that can be categorized into two broad categories:
        \begin{itemize}
...
\begin{column}{\colwidth}
  ...
  \begin{block}{Methodology: Hierarchical Chain of Thought}
    ...
    \begin{figure}
        \includegraphics[width=0.9\linewidth]{hierarchical-chain-of-thought.png}
        \caption{Comparison: Chain of Thought to Hierarchical Chain of Thought}
    \end{figure}
     
  \end{block}

  \begin{block}{Methodology: Model Training}
  ...
  \begin{block}{Methodology: Inference with Context Pruning}
